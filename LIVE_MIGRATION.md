...
## Initial Setup — Access Control Tables (2026-03-25)

### Description
Creates the `users`, `roles`, `user_roles`, `pages`, and `role_permissions` tables and seeds
the default Admin role, admin user, application pages, and the Driver role.
**After running**: log in at `/login.php` with `admin` / `admin123` and change the password
immediately via Access Control → Users → Edit.
...
CREATE TABLE IF NOT EXISTS `pages` (
    ...
    `operation`   VARCHAR(16)            DEFAULT NULL COMMENT 'manage | view | create | edit | delete',
    `description` VARCHAR(255)           DEFAULT NULL,
    `created_at`  DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
...
('Booking Reports',        '/modules/Bookings/',                      'bookings',       'view',   'View booking history'),
...
-- Remove the old public home-page entry if present
...
SELECT r.id, p.id
FROM `roles` r
JOIN `pages` p ON p.path = '/dashboard.php'
WHERE r.name = 'Driver';
```
